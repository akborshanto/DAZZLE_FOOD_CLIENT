...
const Pagination = () => {
  ...
  /* 5 */
  const [itemPerPage, setItemPerPage] = useState(9); //5
  //const itemPage=10;//1
  ...
  useEffect(() => {
    fetch(
      `https://querypdgateway.vercel.app/addQuariess?page=${currentPage}&size=${itemPerPage}`
    )
      .then((res) => res.json())
  ...
  const searchQueryFilter = pagi.filter((queryItem) =>
    queryItem?.pdName?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center">
        {searchQueryFilter.map((pagin) => (
          <PagiCard pagin={pagin}></PagiCard>
        ))}
      </div>
      ...
      <div className="paginaton mx-auto my-8 text-center">
        <h1 className="text-bold text-red-400 text-xl">
